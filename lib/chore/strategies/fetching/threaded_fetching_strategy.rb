require 'chore/strategies/fetching/batcher'
module Chore
  module Strategy
    module Fetching
      class ThreadedFetchingStrategy #:nodoc
        ...
        # If the ThreadedFetchingStrategy is currently running <tt>stop!</tt> will begin signalling it to stop
        ...
        # Returns whether or not the ThreadedFetchingStrategy is running or not
        def running?
          @running
        end
        ...
        def start_consumer_thread(queue)
          t = Thread.new(queue) do |tQueue|
            begin
              ...
              Chore.logger.error "ThreadedFetchingStrategy#consumer thread raised an exception: #{e.inspect} at #{e.backtrace}"
            end
          end
          t
        end

      end #ThreadedFetchingStrategy
    end
  end
end #Chore
